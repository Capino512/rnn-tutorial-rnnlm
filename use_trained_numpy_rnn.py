...
def load_trained_model():
    ...
    print 'start loading...'
    load_model_parameters_numpy(Config._MODEL_FILE, model)
    print 'load over!'
    return model
...
def generate_sentences(model):
    ...
    for i in range(num_sentences):
        ...
        while len(sent) < senten_min_length:
            sent = generate_sentence_by_numpy_rnn(model, 50, word_to_index, index_to_word, Config._SENTENCE_START_TOKEN,
            	Config._SENTENCE_END_TOKEN, Config._UNKNOWN_TOKEN)
        print " ".join(sent)

def generate_sentence_by_numpy_rnn(model, max_length, word_to_index, index_to_word, sentence_start_token, sentence_end_token, unknown_token):
